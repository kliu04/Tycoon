...
io.on("connection", (socket) => {
  ...
  socket.on("player:setUsername", (username) => {
    console.log(`Player with id ${socket.id} has set username to ${username}`);
  ...
  socket.on("room:join", (key, callback) => {
    if (isValidRoom(key)) {
      ...
      console.log(`{Player ${player.username} has joined room ${game.name}`);
    } else {
      callback(false);
    }
  });
  ...
  socket.on("game:start", () => {
    ...
    console.log(`Game with id ${game.key} and name ${game.name} has started.`);
